...
public class manageRequest1 extends AppCompatActivity {
    ...
    private DatabaseReference productsRef, selfproductsRef,categoryRef,pref,catref;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        selfproductsRef = FirebaseDatabase.getInstance().getReference("allRequests");


        productsRef = FirebaseDatabase.getInstance().getReference().child("all_confirmed_supplies");
        categoryRef = FirebaseDatabase.getInstance().getReference("all_confirmed_suppliesCategory");
...
//                        .into(manageProductImage);
                    ...
                    Toast.makeText(manageRequest1.this,"INVALID DATABASE\t no records",Toast.LENGTH_LONG).show();
                }}
...
//        }
        ...
        if (code.getText().toString().isEmpty() || !isValidCode(code.getText().toString())){
            ...
            loadingBar.show();
            productsRef.child(productID).child("status").setValue("PAID");
           ...
           /* int mm = Integer.parseInt(m.trim());
            ...
            productsRef.child(productID).child("status").setValue("PAID");
            productsRef.child(productID).child("price").setValue(price);
            productsRef.child(productID).child("productName").setValue(manageProductName.getText().toString());
            productsRef.child(productID).child("quantity").setValue(manageProductPrice.getText().toString()); //quantity
            productsRef.child(productID).child("productID").setValue(productID);
            productsRef.child(productID).child("ID").setValue(iD);
            productsRef.child(productID).child("mpesa").setValue(code.getText().toString());
            productsRef.child(productID).child("productDescription").setValue(manageProductDescription.getText().toString());
            productsRef.child(productID).child("productCategory").setValue(manageProductCategory.getText().toString());



            selfproductsRef.child(suppliername).child(productID).child("status").setValue("PAID");
            selfproductsRef.child(suppliername).child(productID).child("price").setValue(price);
            selfproductsRef.child(suppliername).child(productID).child("productName").setValue(manageProductName.getText().toString());
            selfproductsRef.child(suppliername).child(productID).child("quantity").setValue(manageProductPrice.getText().toString()); //quantity
            selfproductsRef.child(suppliername).child(productID).child("productID").setValue(productID);
            selfproductsRef.child(suppliername).child(productID).child("ID").setValue(iD);
            selfproductsRef.child(suppliername).child(productID).child("mpesa").setValue(code.getText().toString());
            selfproductsRef.child(suppliername).child(productID).child("productDescription").setValue(manageProductDescription.getText().toString());
            selfproductsRef.child(suppliername).child(productID).child("productCategory").setValue(manageProductCategory.getText().toString());
            ...
            Toast.makeText(this," Supplier payed and status Updated Successfully", Toast.LENGTH_SHORT).show();
    ...
    private void sendUserToMainActivity() {
        Intent mainIntent=new Intent(manageRequest1.this,com.example.tenthskate.Finance.Activity. MainActivity.class);
    ...
    private void admin(final String category,final String description, final String name, final String price,final String qprice,
            ...
            progressDialog.show();
            StringRequest request = new StringRequest(Request.Method.POST,LoginActivity.ip
                    +"supply-orders.php", new Response.Listener<String>() {
                @Override
